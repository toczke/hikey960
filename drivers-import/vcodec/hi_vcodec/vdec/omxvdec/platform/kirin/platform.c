...
static const struct proc_ops gProcOps =
{
    .proc_open    = DRV_PROC_Open,
    .proc_read    = seq_read,
    .proc_write   = DRV_PROC_Write,
...
static inline HI_S32 DRV_PROC_Create_Node(DRV_PROC_FUNC *proc, HI_CHAR *proc_name, drv_proc_read_fn pfn_read, drv_proc_write_fn pfn_write)
{
    ...
    entry->proc_fops  = &gProcOps;
    entry->data       = proc;
#else
    entry = proc_create_data(proc->proc_name, 0644, HI_NULL, &gProcOps, (void*)proc);
    if (!entry)
    {
        return HI_FAILURE;
    }
