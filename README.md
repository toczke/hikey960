...
| **Hardware PWM** (Pin 28) | `[VERIFIED ON HARDWARE — docs/05-GPIO_EXPANSION_HEADER.md]` | Hardware PWM controller driver (`pwm-hisi.c`) and clock tree integration (`HI3660_CLK_GATE_PWM` @ 20 MHz). Exported via Linux PWM subsystem (`/sys/class/pwm/pwmchip0/pwm0`) on physical Pin 28 (`GPIO_182` / `LCD_BL_PWM`). Verified 1 kHz @ 50% duty cycle on live silicon. |
| **Audio Pathways** | `[VERIFIED ON HARDWARE — docs/07-MULTIMEDIA_AND_GPU.md §2]` | HDMI digital audio enabled via ADV7533 bridge DAI (`CONFIG_DRM_I2C_ADV7511_AUDIO=y`). Bluetooth audio via TI WL1837 (`uart4`). Hi6402 analog codec confirmed absent from SBC. LS expansion pins 16, 18, 20, 22 routed to Kirin ASP/I2S0 for external mezzanine DACs. |
