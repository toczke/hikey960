...
| **Audio Pathways** | `[VERIFIED ON HARDWARE — docs/07-MULTIMEDIA_AND_GPU.md §4]` | HDMI digital audio operational via Kirin 960 I2S controller (`hi3660-i2s`) and `simple-audio-card` bound to ADV7533 transmitter (`card 0: hikeyhdmi`, `plughw:0,0`). Safe ASP clock gating in SCTRL prevents bus faults. Real-time PCM audio playback verified on physical Philips HDMI TV using `ffmpeg` and `aplay` (03. Mii Channel.mp3) with 0 SError and 0 DMA-BUF leaks. |
