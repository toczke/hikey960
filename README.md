...
| **Video Codec** (VPU/hi_vcodec) | `[VERIFIED ON HARDWARE — tests/results/]` | Hardware video decoding operational on Kirin 960 VDH silicon. Verified 60/60 frames with SSIM > 0.985 vs CPU reference on VP8 (`vp8_720p30`), HEVC Main/Main10 (`hevc_main_720p30`, `hevc_main_1080p30`, `hevc_main10_1080p30`), and MPEG-2 (`mpeg2_720p30`). H.264 High/Baseline and MPEG-4 decoding partially operational (open bug on DPB queue management/early EOS). Hardware encoder (VENC) node disabled pending C glue layer. Full evidence and logs: [`tests/results/`](tests/results/) and [`docs/07-MULTIMEDIA_AND_GPU.md §3`](docs/07-MULTIMEDIA_AND_GPU.md). |
