...
| **Video Codec** (VPU/hi_vcodec) | `[VERIFIED ON HARDWARE — tests/results/]` | Hardware video decoding operational on Kirin 960 VDH silicon: 60/60 frames with SSIM > 0.985 vs CPU reference on VP8, HEVC Main/Main10, and MPEG-2 (H.264 partial on DPB buffer wait; UAF teardown bug fixed). Hardware video encoding operational on Kirin 960 VEDU silicon: verified H.264 up to 4K UHD 3840×2160 (17.4 FPS), 1080p30 (85.6–116 FPS), and 4× concurrent 1080p30 (5/5 loop PASS, ~31 FPS/stream, ~124 FPS aggregate). H.265/HEVC encode open bug (frame IRQ timeout). Full details in [`docs/07-MULTIMEDIA_AND_GPU.md §3`](docs/07-MULTIMEDIA_AND_GPU.md#3-hardware-video-acceleration-vpu--hi_vcodec). |
