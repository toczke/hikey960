...
*   **The Private SMMU & TrustZone Firewall Challenge:**
    ...
    *   **DRM Driver Minor Mismatch Fix:** Removed `DRIVER_RENDER` from `kirin_drm_driver` because the Kirin DPE is purely a display controller without render ops; this eliminates the `drm_file.c:329` kernel warning and allows userspace to open `/dev/dri/card*` without `-EINVAL`.
